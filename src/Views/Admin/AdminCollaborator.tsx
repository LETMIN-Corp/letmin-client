...
const AdminCollaborator: React.FC = () => {
    ...
    const filterCollaborators = (search: string) => {
        ...
        const filteredUsers = allUsers.filter((user: ICollaborator) => {
        ...
        <AdminDefault>
            ...
            {openModal && (
                <CollaboratorForm
                    isDisabled={true}
                    collaborators={users}
                    selectedCollaboratorKey={selectedUserKey}
    ...
    return (
        <div className="text-sm bg-lilac py-2 px-1 md:px-2 rounded-sm flex items-center justify-between mt-2">
            <span className="w-5/12 md:w-7/12 pr-1">
                <Link
                    className="font-medium text-primary hover:text-bright-purple"
                    to={`/admin/combinations/${collaborator._id}`}
                >
                    {collaborator.name}
                </Link>
            </span>
...
const CollaboratorForm: React.FC<CollaboratorFormInterface> = ({
    isDisabled,
    collaborators,
