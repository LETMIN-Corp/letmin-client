...
const CompanyTalentBank = () => {
    ...
    useEffect(() => {
        ...
        let filteredTalents = allTalents.filter((talent : { name : string }) => {
            ...
            <div className='p-5 min-h-90'>
                ...
                <div className='w-full flex items-center justify-between mt-5'>
                    <div className='max-w-sm w-full relative mr-2'>
                        <input onChange={ (e) => { setSearch(e.target.value) } } type='text' placeholder='Buscar' className='w-full mr-3 pl-2 pr-8 py-1 border-2 border-dark-purple rounded-md' name='search' id='search' />
                        ...
                        <div className='mt-5 grid md:grid-cols-2 lg:grid-cols-4 gap-4'>
                            {
                                ! filteredTalents.length && (
                                    <div className='mt-5 text-center md:text-left text-dark-purple text-lg font-medium'>Nenhum talento encontrado</div>
                                )
                            }
                            {
                                filteredTalents.map((user, key) => <TalentBankCard key={ key } user={ user } />)                                    
                            }
                        </div>
                    )
                }
